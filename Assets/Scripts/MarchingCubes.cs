...
public class MarchingCubes : MonoBehaviour
{
    ...
    {
        int diameter = Mathf.CeilToInt(Radius) * 2 + 2;
        Densities = new float[diameter * Detail, diameter * Detail, diameter * Detail];

        Vector3 center = new Vector3(Densities.GetLength(0) / 2.0f, Densities.GetLength(1) / 2.0f, Densities.GetLength(2) / 2.0f) / Detail;
        for (int x = 0; x < Densities.GetLength(0); x++)
            for (int y = 0; y < Densities.GetLength(1); y++)
                for (int z = 0; z < Densities.GetLength(2); z++)
                {
                    Vector3 point = new Vector3(x, y, z) / Detail - center;
                    Densities[x, y, z] = Mathf.Clamp(Radius - point.magnitude, -1.0f, 1.0f);
                }
    ...
    public void RecalculateMesh()
    {
        ...
        for (int x = 0; x < Densities.GetLength(0) - 1; x++)
            for (int y = 0; y < Densities.GetLength(1) - 1; y++)
                for (int z = 0; z < Densities.GetLength(2) - 1; z++)
                {
                    ...
                    {
                        Vector3Int point = offset + new Vector3Int((cubeIndex & 1) >> 0, (cubeIndex & 2) >> 1, (cubeIndex & 4) >> 2);
                        if (Densities[point.x, point.y, point.z] > 0.0f)
                        {
                            faceIndex += 1 << cubeIndex;
                        }
                    }

                    int[] triangles = TriangleTable[faceIndex];
                    for (int i = 0; i < triangles.Length; i++)
                    {
                        int vertexIndex = vertices.Count;
                        int[] edge = EdgeVertexIndices[triangles[i]];
                        Vector3Int pointA = offset + new Vector3Int((edge[0] & 1) >> 0, (edge[0] & 2) >> 1, (edge[0] & 4) >> 2);
                        Vector3Int pointB = offset + new Vector3Int((edge[1] & 1) >> 0, (edge[1] & 2) >> 1, (edge[1] & 4) >> 2);
                        float densityA = Densities[pointA.x, pointA.y, pointA.z];
                        float densityB = Densities[pointB.x, pointB.y, pointB.z];
                        vertices.Add(Vector3.Lerp(pointA, pointB, (0.0f - densityA) / (densityB - densityA)) / Detail - center);
    ...
    private void OnDrawGizmos()
    {
        if (Densities != null)
        {
            Vector3 center = new(Densities.GetLength(0) / 2.0f, Densities.GetLength(1) / 2.0f, Densities.GetLength(2) / 2.0f);
            ...
            for (int x = 0; x < Densities.GetLength(0); x++)
                for (int y = 0; y < Densities.GetLength(1); y++)
                    for (int z = 0; z < Densities.GetLength(2); z++)
                    {
                        ...
                        Color color = Color.white * Densities[x, y, z];
                        color.a = 1.0f;
                        Gizmos.color = color;
    ...
    */
    ...
    private int[][] EdgeVertexIndices = new[] {
        new[] { 0, 1 },
        new[] { 1, 3 },
        new[] { 3, 2 },
        new[] { 2, 0 },
    ...
    private int[][] TriangleTable = new int[][] {
        ...
        new[] { 0, 9, 1, 8, 7, 4, 11, 3, 2 },
        new[] { 7, 4, 11, 11, 4, 2, 2, 4, 9, 2, 9, 1 },
        new[] { 4, 8, 7, 2, 1, 10 },
        new[] { 7, 4, 3, 3, 4, 0, 10, 2, 1 },
        new[] { 10, 2, 9, 9, 2, 0, 7, 4, 8 },
        new[] { 10, 2, 3, 10, 3, 4, 3, 7, 4, 9, 10, 4 },
        new[] { 1, 10, 3, 3, 10, 11, 4, 8, 7 },
        new[] { 10, 11, 1, 11, 7, 4, 1, 11, 4, 1, 4, 0 },
        ...
        new[] { 5, 1, 4, 1, 2, 11, 4, 1, 11, 4, 11, 8 },
        new[] { 1, 10, 2, 5, 9, 4 },
        new[] { 9, 4, 5, 0, 3, 8, 2, 1, 10 },
        new[] { 2, 5, 10, 2, 4, 5, 2, 0, 4 },
        new[] { 10, 2, 5, 5, 2, 4, 4, 2, 3, 4, 3, 8 },
        new[] { 11, 3, 10, 10, 3, 1, 4, 5, 9 },
        new[] { 4, 5, 9, 10, 0, 1, 10, 8, 0, 10, 11, 8 },
        new[] { 11, 3, 0, 11, 0, 5, 0, 4, 5, 10, 11, 5 },
        ...
        new[] { 2, 11, 7, 2, 7, 9, 7, 5, 9, 0, 2, 9 },
        new[] { 2, 11, 3, 7, 0, 8, 7, 1, 0, 7, 5, 1 },
        new[] { 2, 11, 1, 11, 7, 1, 7, 5, 1 },
        new[] { 8, 7, 9, 9, 7, 5, 2, 1, 10 },
        new[] { 10, 2, 1, 3, 9, 0, 3, 5, 9, 3, 7, 5 },
        new[] { 7, 5, 8, 5, 10, 2, 8, 5, 2, 8, 2, 0 },
        new[] { 10, 2, 5, 2, 3, 5, 3, 7, 5 },
        new[] { 8, 7, 5, 8, 5, 9, 11, 3, 10, 3, 1, 10 },
        new[] { 5, 11, 7, 10, 11, 5, 1, 9, 0 },
        new[] { 11, 5, 10, 7, 5, 11, 8, 3, 0 },
        new[] { 5, 11, 7, 10, 11, 5 },
        new[] { 6, 7, 11 },
        new[] { 7, 11, 6, 3, 8, 0 },
        new[] { 6, 7, 11, 0, 9, 1 },
        ...
        new[] { 11, 6, 4, 11, 4, 8, 10, 2, 9, 2, 0, 9 },
        new[] { 10, 4, 9, 6, 4, 10, 11, 2, 3 },
        new[] { 4, 8, 3, 4, 3, 10, 3, 1, 10, 6, 4, 10 },
        new[] { 1, 10, 0, 10, 6, 0, 6, 4, 0 },
        new[] { 4, 10, 6, 9, 10, 4, 0, 8, 3 },
        new[] { 4, 10, 6, 9, 10, 4 },
        new[] { 6, 7, 11, 4, 5, 9 },
        new[] { 4, 5, 9, 7, 11, 6, 3, 8, 0 },
        new[] { 1, 0, 5, 5, 0, 4, 11, 6, 7 },
        new[] { 11, 6, 7, 5, 8, 4, 5, 3, 8, 5, 1, 3 },
        new[] { 3, 2, 7, 7, 2, 6, 9, 4, 5 },
        new[] { 5, 9, 4, 0, 7, 8, 0, 6, 7, 0, 2, 6 },
        new[] { 3, 2, 6, 3, 6, 7, 1, 0, 5, 0, 4, 5 },
        new[] { 6, 1, 2, 5, 1, 6, 4, 7, 8 },
        new[] { 10, 2, 1, 6, 7, 11, 4, 5, 9 },
        new[] { 0, 3, 8, 4, 5, 9, 11, 6, 7, 10, 2, 1 },
        new[] { 7, 11, 6, 2, 5, 10, 2, 4, 5, 2, 0, 4 },
        new[] { 8, 4, 7, 5, 10, 6, 3, 11, 2 },
        new[] { 9, 4, 5, 7, 10, 6, 7, 1, 10, 7, 3, 1 },
        new[] { 10, 6, 5, 7, 8, 4, 1, 9, 0 },
        new[] { 4, 3, 0, 7, 3, 4, 6, 5, 10 },
        new[] { 10, 6, 5, 8, 4, 7 },
        new[] { 9, 6, 5, 9, 11, 6, 9, 8, 11 },
        new[] { 11, 6, 3, 3, 6, 0, 0, 6, 5, 0, 5, 9 },
        new[] { 11, 6, 5, 11, 5, 0, 5, 1, 0, 8, 11, 0 },
        new[] { 11, 6, 3, 6, 5, 3, 5, 1, 3 },
        new[] { 9, 8, 5, 8, 3, 2, 5, 8, 2, 5, 2, 6 },
        new[] { 5, 9, 6, 9, 0, 6, 0, 2, 6 },
        new[] { 1, 6, 5, 2, 6, 1, 3, 0, 8 },
        new[] { 1, 6, 5, 2, 6, 1 },
        new[] { 2, 1, 10, 9, 6, 5, 9, 11, 6, 9, 8, 11 },
        new[] { 9, 0, 1, 3, 11, 2, 5, 10, 6 },
        new[] { 11, 0, 8, 2, 0, 11, 10, 6, 5 },
        new[] { 3, 11, 2, 5, 10, 6 },
        new[] { 1, 8, 3, 9, 8, 1, 5, 10, 6 },
        new[] { 6, 5, 10, 0, 1, 9 },
        new[] { 8, 3, 0, 5, 10, 6 },
        new[] { 6, 5, 10 },
        new[] { 10, 5, 6 },
        ...
        new[] { 1, 0, 9, 2, 11, 3, 6, 10, 5 },
        new[] { 5, 6, 10, 11, 1, 2, 11, 9, 1, 11, 8, 9 },
        new[] { 5, 6, 1, 1, 6, 2 },
        new[] { 5, 6, 1, 1, 6, 2, 8, 0, 3 },
        new[] { 6, 9, 5, 6, 0, 9, 6, 2, 0 },
        new[] { 6, 2, 5, 2, 3, 8, 5, 2, 8, 5, 8, 9 },
        new[] { 3, 6, 11, 3, 5, 6, 3, 1, 5 },
        new[] { 8, 0, 1, 8, 1, 6, 1, 5, 6, 11, 8, 6 },
        new[] { 11, 3, 6, 6, 3, 5, 5, 3, 0, 5, 0, 9 },
        new[] { 5, 6, 9, 6, 11, 9, 11, 8, 9 },
        new[] { 5, 6, 10, 7, 4, 8 },
        new[] { 0, 3, 4, 4, 3, 7, 10, 5, 6 },
        new[] { 5, 6, 10, 4, 8, 7, 0, 9, 1 },
        new[] { 6, 10, 5, 1, 4, 9, 1, 7, 4, 1, 3, 7 },
        new[] { 7, 4, 8, 6, 10, 5, 2, 11, 3 },
        new[] { 10, 5, 6, 4, 11, 7, 4, 2, 11, 4, 0, 2 },
        new[] { 4, 8, 7, 6, 10, 5, 3, 2, 11, 1, 0, 9 },
        new[] { 1, 2, 10, 11, 7, 6, 9, 5, 4 },
        new[] { 2, 1, 6, 6, 1, 5, 8, 7, 4 },
        new[] { 0, 3, 7, 0, 7, 4, 2, 1, 6, 1, 5, 6 },
        new[] { 8, 7, 4, 6, 9, 5, 6, 0, 9, 6, 2, 0 },
        new[] { 7, 2, 3, 6, 2, 7, 5, 4, 9 },
        new[] { 4, 8, 7, 3, 6, 11, 3, 5, 6, 3, 1, 5 },
        new[] { 5, 0, 1, 4, 0, 5, 7, 6, 11 },
        new[] { 9, 5, 4, 6, 11, 7, 0, 8, 3 },
        new[] { 11, 7, 6, 9, 5, 4 },
        new[] { 6, 10, 4, 4, 10, 9 },
        new[] { 6, 10, 4, 4, 10, 9, 3, 8, 0 },
        new[] { 0, 10, 1, 0, 6, 10, 0, 4, 6 },
        new[] { 6, 10, 1, 6, 1, 8, 1, 3, 8, 4, 6, 8 },
        new[] { 9, 4, 10, 10, 4, 6, 3, 2, 11 },
        new[] { 2, 11, 8, 2, 8, 0, 6, 10, 4, 10, 9, 4 },
        new[] { 11, 3, 2, 0, 10, 1, 0, 6, 10, 0, 4, 6 },
        new[] { 6, 8, 4, 11, 8, 6, 2, 10, 1 },
        new[] { 4, 1, 9, 4, 2, 1, 4, 6, 2 },
        new[] { 3, 8, 0, 4, 1, 9, 4, 2, 1, 4, 6, 2 },
        new[] { 6, 2, 4, 4, 2, 0 },
        new[] { 3, 8, 2, 8, 4, 2, 4, 6, 2 },
        new[] { 4, 6, 9, 6, 11, 3, 9, 6, 3, 9, 3, 1 },
        new[] { 8, 6, 11, 4, 6, 8, 9, 0, 1 },
        new[] { 11, 3, 6, 3, 0, 6, 0, 4, 6 },
        new[] { 8, 6, 11, 4, 6, 8 },
        new[] { 10, 7, 6, 10, 8, 7, 10, 9, 8 },
        new[] { 3, 7, 0, 7, 6, 10, 0, 7, 10, 0, 10, 9 },
        new[] { 6, 10, 7, 7, 10, 8, 8, 10, 1, 8, 1, 0 },
        new[] { 6, 10, 7, 10, 1, 7, 1, 3, 7 },
        new[] { 3, 2, 11, 10, 7, 6, 10, 8, 7, 10, 9, 8 },
        new[] { 2, 9, 0, 10, 9, 2, 6, 11, 7 },
        new[] { 0, 8, 3, 7, 6, 11, 1, 2, 10 },
        new[] { 7, 6, 11, 1, 2, 10 },
        new[] { 2, 1, 9, 2, 9, 7, 9, 8, 7, 6, 2, 7 },
        new[] { 2, 7, 6, 3, 7, 2, 0, 1, 9 },
        new[] { 8, 7, 0, 7, 6, 0, 6, 2, 0 },
        new[] { 7, 2, 3, 6, 2, 7 },
        new[] { 8, 1, 9, 3, 1, 8, 11, 7, 6 },
        new[] { 11, 7, 6, 1, 9, 0 },
        new[] { 6, 11, 7, 0, 8, 3 },
        new[] { 11, 7, 6 },
        new[] { 7, 11, 5, 5, 11, 10 },
        ...
        new[] { 7, 11, 10, 7, 10, 5, 3, 8, 1, 8, 9, 1 },
        new[] { 5, 2, 10, 5, 3, 2, 5, 7, 3 },
        new[] { 5, 7, 10, 7, 8, 0, 10, 7, 0, 10, 0, 2 },
        new[] { 0, 9, 1, 5, 2, 10, 5, 3, 2, 5, 7, 3 },
        new[] { 9, 7, 8, 5, 7, 9, 10, 1, 2 },
        new[] { 1, 11, 2, 1, 7, 11, 1, 5, 7 },
        new[] { 8, 0, 3, 1, 11, 2, 1, 7, 11, 1, 5, 7 },
        new[] { 7, 11, 2, 7, 2, 9, 2, 0, 9, 5, 7, 9 },
        new[] { 7, 9, 5, 8, 9, 7, 3, 11, 2 },
        new[] { 3, 1, 7, 7, 1, 5 },
        new[] { 8, 0, 7, 0, 1, 7, 1, 5, 7 },
        new[] { 0, 9, 3, 9, 5, 3, 5, 7, 3 },
        new[] { 9, 7, 8, 5, 7, 9 },
        new[] { 8, 5, 4, 8, 10, 5, 8, 11, 10 },
        new[] { 0, 3, 11, 0, 11, 5, 11, 10, 5, 4, 0, 5 },
        new[] { 1, 0, 9, 8, 5, 4, 8, 10, 5, 8, 11, 10 },
        new[] { 10, 3, 11, 1, 3, 10, 9, 5, 4 },
        new[] { 3, 2, 8, 8, 2, 4, 4, 2, 10, 4, 10, 5 },
        new[] { 10, 5, 2, 5, 4, 2, 4, 0, 2 },
        new[] { 5, 4, 9, 8, 3, 0, 10, 1, 2 },
        new[] { 2, 10, 1, 4, 9, 5 },
        new[] { 8, 11, 4, 11, 2, 1, 4, 11, 1, 4, 1, 5 },
        new[] { 0, 5, 4, 1, 5, 0, 2, 3, 11 },
        new[] { 0, 11, 2, 8, 11, 0, 4, 9, 5 },
        new[] { 5, 4, 9, 2, 3, 11 },
        ...
        new[] { 11, 10, 7, 10, 1, 0, 7, 10, 0, 7, 0, 4 },
        new[] { 3, 10, 1, 11, 10, 3, 7, 8, 4 },
        new[] { 3, 2, 10, 3, 10, 4, 10, 9, 4, 7, 3, 4 },
        new[] { 9, 2, 10, 0, 2, 9, 8, 4, 7 },
        new[] { 3, 4, 7, 0, 4, 3, 1, 2, 10 },
        new[] { 7, 8, 4, 10, 1, 2 },
        new[] { 7, 11, 4, 4, 11, 9, 9, 11, 2, 9, 2, 1 },
        new[] { 1, 9, 0, 4, 7, 8, 2, 3, 11 },
